...
import android.Manifest; import android.app.*; import android.os.*; import android.content.*; import android.content.pm.PackageManager; import android.location.*; import android.net.Uri; import android.widget.*; import org.json.*; import java.util.*;
...
public class JobDetailActivity extends Activity {
    ...
    private void render(){ ScrollView sc=new ScrollView(this);p=Ui.page(this);sc.addView(p);if(demo){TextView d=Ui.text(this,"MODALITÀ DEMO GOOGLE PLAY · dati sintetici, nessun dato aziendale reale");d.setTextColor(Ui.GREEN);p.addView(d);}p.addView(Ui.title(this,job.optString("client_name","Incarico")));p.addView(Ui.text(this,job.optString("work_date")+" · "+job.optString("start_time")+" – "+job.optString("end_time")));String location=job.optString("location","");
        p.addView(Ui.text(this,location));
        if(!location.isEmpty()&&!demo){Button nav=Ui.button(this,"Avvia navigatore");p.addView(nav);nav.setOnClickListener(v->openNavigator(location));}
        p.addView(Ui.text(this,"Stato: "+statusLabel(job.optString("status"))));String notes=job.optString("notes","");if(!notes.isEmpty())p.addView(Ui.text(this,"Note: "+notes));String s=job.optString("status");if("assigned".equals(s)) action("Accetta incarico","accepted",false);if("accepted".equals(s)) action("Inizia lavoro","in_progress",true);if("in_progress".equals(s)) action("Termina lavoro","completed",true);setContentView(sc); }
    private String statusLabel(String s){if("assigned".equals(s))return "Assegnato";if("accepted".equals(s))return "Accettato";if("in_progress".equals(s))return "In corso";if("completed".equals(s))return "Completato";if("cancelled".equals(s))return "Annullato";return s;}
    private void openNavigator(String address){
        try{Intent i=new Intent(Intent.ACTION_VIEW,Uri.parse("geo:0,0?q="+Uri.encode(address)));startActivity(Intent.createChooser(i,"Raggiungi la sede con"));}
        catch(Exception e){startActivity(new Intent(Intent.ACTION_VIEW,Uri.parse("https://www.google.com/maps/dir/?api=1&destination="+Uri.encode(address))));}
    }
